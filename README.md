...
[![Deploy to Bluemix](https://bluemix.net/deploy/button.png)](https://bluemix.net/deploy?repository=https://github.com/IBM-Bluemix/easy-count-server)
...
1. Execute full Maven build to create the `target/EasyCountServer.war` file:
    ```bash
    $ mvn clean install
    ```
...
2. Download and install Liberty, then use it to run the built application from step 1:
    ...
    Once the server is running, the application will be available under [http://localhost:9080/EasyCountServer](http://localhost:9080/EasyCountServer).

Use the following command to run the built application in Bluemix:
    ```bash
    $ cf push <appname> -p target/EasyCountServer.war
    ```
...
4. Run your application locally on Liberty:
  - Right click on the `EasyCountServer` sample and select `Run As` -> `Run on Server` option
  - Find and select the localhost Liberty server and press `Finish`
  - In a few seconds, your application should be running at http://localhost:9080/EasyCountServer/
